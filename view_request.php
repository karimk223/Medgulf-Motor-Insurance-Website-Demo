...
if(isset($_GET['pdf']) && $_GET['pdf'] == 1){
    ...
    [$premium, $allRiskPremium, $effPlan, $effRate, $effMin] = calcTotalPremium($types, $r_car_value, $r_year_built, $r_package_option);

    $full_name = trim($c_fn . ' ' . $c_mn . ' ' . $c_ln);
    ...
    $pdf->addLine(60, 708, 12, 'Policy / Request #: ' . $r_id);
    $pdf->addLine(60, 690, 12, 'Issue Date: ' . date('Y-m-d'));
    $pdf->addLine(60, 672, 12, 'Inception Date: ' . ($r_inception ?? ''));
    $pdf->addLine(60, 654, 12, 'Expiration Date: ' . $exp_date);

    $pdf->addLine(60, 624, 14, 'Client Information');
    $pdf->addLine(60, 604, 12, 'Name: ' . $full_name);
    ...
    $pdf->addLine(60, 506, 12, 'Model: ' . ($v_model ?? ''));
    $pdf->addLine(60, 490, 12, 'License Plate: ' . ($v_plate ?? ''));
    $pdf->addLine(60, 474, 12, 'Vehicle Number: ' . ($v_number ?? ''));
    $pdf->addLine(60, 458, 12, 'Year Built: ' . (($r_year_built ?? '') !== '' ? $r_year_built : '—'));
    $pdf->addLine(60, 442, 12, 'Car Value: $' . number_format((float)$r_car_value, 2));

    $pdf->addLine(60, 412, 14, 'Insurance Details');
    $pdf->addLine(60, 392, 12, 'Types: ' . (count($types) ? implode(', ', $types) : 'N/A'));

    $y2 = 376;
    if(in_array('All Risk', $types)){
        $pdf->addLine(60, $y2, 12, 'All Risk Plan: ' . ($effPlan ?: '—'));
        $y2 -= 16;
        $pdf->addLine(60, $y2, 12, 'All Risk Premium: $' . number_format((float)$allRiskPremium, 2));
        $y2 -= 16;
    }

    $pdf->addLine(60, $y2, 12, 'Total Premium: $' . number_format((float)$premium, 2));

    $footer_y = 308;
    $pdf->addLine(60, $footer_y, 10, 'Status: ' . ($r_status ?? 'Pending'));
    $pdf->addLine(60, $footer_y-14, 10, 'Processed By: ' . ($processed_by_name !== '' ? $processed_by_name : 'N/A'));
